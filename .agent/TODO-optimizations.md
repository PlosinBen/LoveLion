...
### ~~H1. 缺少 Production docker-compose~~（已完成）

已建立 `docker-compose.prod.yml`、`.env.prod.example`，並改進 `backend/Dockerfile`。

---

### ~~H2. Gin 未設定 Release Mode~~（已完成）

透過 `docker-compose.prod.yml` 設定 `GIN_MODE=release` 環境變數，Gin 自動讀取。
