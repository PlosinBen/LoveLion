...
## P1 — HIGH（強烈建議在 Release 前處理）

### H1. 缺少 Production docker-compose

目前的 `docker-compose.yml` 用 `go run` 和 `npm run dev`，不適合 production。

**建議**：建立 `docker-compose.prod.yml`，使用 Dockerfile multi-stage build 出的 compiled binary 和 `nuxt build` 產出。

---

### H2. Gin 未設定 Release Mode

`backend/main.go` 沒有呼叫 `gin.SetMode(gin.ReleaseMode)`，production 會輸出 debug 資訊。

---

### H3. GORM log level 太高

`backend/internal/database/database.go` 設為 `logger.Info`，會印出所有 SQL。

**建議**：production 改為 `logger.Silent` 或 `logger.Warn`，可透過環境變數控制。

---

### H4. 缺少自訂錯誤頁面

前端沒有 404 / error page，使用者看到的是 Nuxt 預設錯誤頁。

**建議**：建立 `frontend/error.vue` 和 catch-all route。

---

### H5. 缺少 favicon 和 robots.txt

`frontend/public/` 只有 logo，缺少基本的 `favicon.ico` 和 `robots.txt`。
