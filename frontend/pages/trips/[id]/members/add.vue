<template>
  <div class="flex flex-col gap-6">
    <header class="flex justify-between items-center">
      ...
      <h1 class="text-xl font-bold">新增成員</h1>
      <div class="w-10"></div>
    </header>

    <div class="bg-neutral-900 rounded-2xl p-5 border border-neutral-800">
      <BaseInput v-model="name" placeholder="成員名稱" input-class="mb-4" @keyup.enter="handleSubmit" :auto-focus="true" />
...
const handleSubmit = async () => {
  ...
  try {
    await api.post(`/api/trips/${route.params.id}/members`, { name: name.value.trim() })
