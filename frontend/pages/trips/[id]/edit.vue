...
watch(() => form.value.currencies, (newVal) => {
  if (newVal.length > 0 && !newVal.includes(form.value.base_currency)) {
    // Only auto-switch if the current base currency is strictly invalid (removed)
    // But initially base_currency might be valid.
    // Wait, if I remove 'TWD' and it was base, I must switch.
...
const fetchTrip = async () => {
  try {
    ...
    // Then set base currency (ensure it exists in list)
    const base = trip.base_currency || 'TWD'
    if (!form.value.currencies.includes(base)) {
        form.value.currencies.push(base)
    }
    form.value.base_currency = base
